...
impl ConnectionLimiter {
    ...
    fn check_api_key_limit(&self, api_key: &str) -> bool {
        ...
        let mut entry = self.api_key_counters.entry(api_key.to_string()).or_insert(0);
        let api_key_limit = conf::Settings::global().websocket.api_key_conn_limit_per_minute;
        
        // 检查是否超过API Key限制
        if *entry >= api_key_limit {
            log::warn!("API Key {} 每分钟连接数超过限制 ({})", api_key, api_key_limit);
            return false;
        }
    ...
    fn check_global_limit(&self) -> bool {
        ...
        let global_limit = conf::Settings::global().websocket.global_conn_limit_per_minute;
    ...
    fn update_minute_if_needed(&self) {
        ...
        if now > current {
            // 尝试更新当前分钟，只有一个线程会成功
            if self.current_minute.compare_exchange(
                current, now, Ordering::SeqCst, Ordering::Relaxed
            ).is_ok() {
                // 清空API Key计数器
    ...
    if let Some(addr) = peer_addr {
        ...
        if api_key == conf::admin_key() {
            ...
            if !CONNECTION_LIMITER.check_api_key_limit(&api_key) {
                log::warn!("API Key连接数限制，拒绝连接: {}, API Key: {}", addr, api_key);
                return;
            }
